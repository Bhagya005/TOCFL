...
export async function GET(request: Request) {
  ...
  const testList = (testRows ?? []).map((r) => {
    ...
    return { date: r.date, test_type: r.test_type, score: r.score, total: r.total, day_index: dayIndex };
  });

  const { data: wordStatsRows } = await supabase
    .from("words")
    .select("id, character")
    .lte("id", plan.unlockedUptoWordId)
    .order("id");

  const progressMap = new Map((progressRows ?? []).map((r) => [r.word_id, r]));

  const wordStats = (wordStatsRows ?? []).map((w) => {
    const p = progressMap.get(w.id);
    return {
      word_id: w.id,
      character: w.character,
      attempts: p ? Number(p.attempts) : 0,
      correct: p ? Number(p.correct) : 0,
    };
  });
  ...
  return NextResponse.json({
    ...
    summary: { known_words: knownWords, accuracy },
